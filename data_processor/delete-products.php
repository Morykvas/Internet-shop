...
try {
    if (!empty($user_id) && !empty($product_id)) {
        ...
        $sqlQuotaQuantity = "SELECT product_quontity FROM quota WHERE user_id = $user_id AND product_id = $product_id";
        ...
        if ($resultQuotaQuantity && mysqli_num_rows($resultQuotaQuantity) > 0) {
            ...
            if ($resultDeleteFromQuota) {
                ...
                $sqlProductQuantity = "SELECT product_quontity FROM products WHERE product_id = $product_id";
                ...
                if ($resultProductQuantity && mysqli_num_rows($resultProductQuantity) > 0) {
                    ...
                    $sqlUpdateProductQuantity = "UPDATE products SET product_quontity = $newProductQuantity WHERE product_id = $product_id";
